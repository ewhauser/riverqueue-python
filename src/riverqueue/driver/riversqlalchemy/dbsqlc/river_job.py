...
INSERT INTO river_job(
    ...
    unnest(:p8\\:\\:text[])\\:\\:river_job_state,
...
JOB_INSERT_FAST_MANY_NO_RETURNING = """-- name: job_insert_fast_many_no_returning \\:execrows
INSERT INTO river_job(
    args,
    kind,
    max_attempts,
...
@dataclasses.dataclass()
class JobInsertFastManyNoReturningParams:
    args: List[Any]
    kind: List[str]
    max_attempts: List[int]
    metadata: List[Any]
...
class Querier:
    ...
    def job_insert_fast_many_no_returning(self, arg: JobInsertFastManyNoReturningParams) -> int:
        result = self._conn.execute(sqlalchemy.text(JOB_INSERT_FAST_MANY_NO_RETURNING), {
            "p1": arg.args,
            "p2": arg.kind,
            "p3": arg.max_attempts,
            "p4": arg.metadata,
